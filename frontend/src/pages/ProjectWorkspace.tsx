    ...
    return (
      ...
      >
        <FileTabs
          ...
          onActivate={(path) => activateTab(paneId, path)}
          onClose={(path) => closeTab(paneId, path)}
          onPromote={(path) => promoteTab(paneId, path)}
          onMove={(path, targetIndex) => moveTab(paneId, path, targetIndex)}
          onDropPaths={(payload) => handleDropPathsOnTabs(paneId, payload)}
          snippetToolbarVisible={paneState.showSnippetToolbar}
          onToggleSnippetToolbar={() => togglePaneSnippetToolbar(paneId)}
        />

        <div
          className="relative flex-1 min-h-0 min-w-0"
          onDragOverCapture={(event) => handlePaneDragOver(event, paneId)}
          onDragLeaveCapture={(event) => handlePaneDragLeave(event, paneId)}
          onDropCapture={(event) => handlePaneDrop(event, paneId)}
        >
          {historyState && paneId === activePaneId ? (
            ...
            <Editor
              ...
              presenceGuestId={principal.guestId}
